...
int main(int argc, char** argv){
  ...
  // Nodehandle to set boolean params on Parameter Server
	ros::NodeHandle n;
	n.setParam("robot_reach_pickup_zone", false);
	n.setParam("robot_reach_dropoff_zone", false);
  ...
  while(run){
    ...
    if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
      if(counter == 0) {
        ...
        n.setParam("robot_at_pickup_zone", true);
      }
      if(counter == 1){ 
        ROS_INFO("Robot reached destination drop-off goal");
        n.setParam("robot_at_dropoff_zone", true);
