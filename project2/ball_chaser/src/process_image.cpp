...
void drive_robot(float lin_x, float ang_z)
{
    ...
    ROS_INFO_STREAM("Moving robot towards the camera");
...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    if(!ball_found){
      ...
      {
        drive_robot(0.0, 0.1);   // turn left bc white_pixel in left region
      ...
      {
      	drive_robot(0.3, 0.0);   // go forward
        ROS_INFO("MOVING FORWARD");
      }
        
      else if (column_index > 2*int(img.width/3))
      {
        drive_robot(0.0, -0.1);   // turn right bc white_pixel in right region
